...
function App() {
  ...
  useEffect(() => {
    const fetchCookbookRecipes = async () => {
      const token = localStorage.getItem('token')
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/recipes/cookbook/`, {
          headers: { Authorization: `Token ${token}` },
        });
        setCookbookRecipes(response.data)
      } catch (err) {
    ...
    const fetchDietRecipes = async () => {
      const token = localStorage.getItem('token')
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/diet/diet-plans/`, {
          headers: { Authorization: `Token ${token}` },
        });
        setDietRecipes(response.data)
      } catch (err) {
  ...
  const handleAddToCookbook = async (recipe) => {
    const token = localStorage.getItem('token')
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/recipes/add-to-cookbook/`, 
        { uri: recipe.uri }, 
        { headers: { Authorization: `Token ${token}` } }
      );
      setCookbookRecipes([...cookbookRecipes, recipe])
  ...
  const handleRemoveFromCookbook = async (myCookbookId) => {
    const token = localStorage.getItem('token')
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/recipes/remove-from-cookbook/${myCookbookId}/`, {
        headers: { Authorization: `Token ${token}` }
      });
      setCookbookRecipes(prevRecipes => 
        prevRecipes.filter(r => r.id !== myCookbookId) 
  ...
  const handleAddToDiet = async (recipe, mealTime, dayOfWeek, date) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/api/diet/add-to-diet/`, 
        { 
          uri: recipe.uri,  //recipe.uri in AddToCookbookView
          meal_time: mealTime,  
          day_of_week: dayOfWeek,  
...
const handleRemoveFromDiet = async (dietPlanMealId) => {
  const token = localStorage.getItem('token')
  try {
      await axios.delete(`${process.env.REACT_APP_API_URL}/api/diet/remove-from-diet/${dietPlanMealId}/`, {
          headers: { Authorization: `Token ${token}` }
      });
